...
module.exports = async function handler(req, res) {
  ...
  try {
    const apiRes = await fetch(`${API_BASE}/api/today-doom`, { signal: AbortSignal.timeout(3000) })
    if (apiRes.ok) {
  ...
  const element = h('div', {
    ...
    h('div', { style: { fontSize: '18px', color: '#888888', marginBottom: '12px', letterSpacing: '3px', display: 'flex' } },
      'EARTH DOOM INDEX'
    ),
    dateStr
      ? h('div', { style: { fontSize: '12px', color: '#555555', marginBottom: '36px', display: 'flex' } }, dateStr)
      : null,
    score !== null
      ? h('div', { style: { display: 'flex', alignItems: 'baseline', marginBottom: '24px' } },
          h('span', { style: { fontSize: '128px', color: dangerInfo.color, lineHeight: 1 } }, String(score)),
          h('span', { style: { fontSize: '36px', color: '#555555', marginLeft: '12px' } }, '/ 100')
        )
      : h('div', { style: { fontSize: '48px', color: '#555555', marginBottom: '24px', display: 'flex' } }, '— / 100'),
    h('div', {
      style: {
  ...
  const imageResponse = new ImageResponse(element, { width: 1200, height: 630, fonts })

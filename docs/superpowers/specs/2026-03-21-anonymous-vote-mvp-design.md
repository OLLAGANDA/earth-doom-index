...
스케줄러는 매일 **UTC 00:01**에 doom 점수를 계산한다. 투표 시간 창은 이를 기준으로 한다.

| 구간 | 시간 (UTC) | 상태 |
|------|-----------|------|
| 투표 오픈 | 00:01 ~ 23:59 | 투표 가능 (재투표 포함) |
| 투표 마감 | 23:59 이후 ~ 다음날 00:01 | 투표 불가, 결과 대기 중 |
| 결과 오픈 | 다음날 00:01 이후 | 어제 예측 결과 표시 |
...
6. 다음날 UTC 00:01 이후 → 어제 예측 결과 표시
...
function getVotePhase(todayDoomDate) {
  ...
  // 현재 UTC 시각과 비교해 'open' | 'closed' | 'result' 반환
  // open:   오늘 날짜이고 UTC 00:01 ~ 23:58
  // closed: 오늘 날짜이고 UTC 23:59 이후
  // result: 내일 이후 (새 doom 점수 계산 완료 시점)
}
```

...
1. 오늘 아직 투표하지 않은 경우 → **UP / DOWN 버튼** 표시
2. 버튼 클릭 → `POST /api/vote` 호출 → 로컬스토리지에 `edi-vote-YYYY-MM-DD: 'up'|'down'` 저장
3. 투표 후 → **집단 예측 결과** 표시 (예: "68% UP ▲ / 32% DOWN ▽, 총 142명")
4. 다음날 재방문 시 → 어제 예측 결과 표시 (예: "어제 67%가 UP 예측 → 실제 ▲ 맞았습니다 ✓")
...
| Method | Path | 설명 |
|--------|------|------|
| `POST /api/vote` | `{ direction: 'up'\|'down', target_date: 'YYYY-MM-DD' }` | 투표 저장 |
...
- 요청: `{ direction: 'up' | 'down', target_date: 'YYYY-MM-DD' }`
- `direction`이 `'up'`/`'down'` 이외의 값이거나 `target_date`가 유효하지 않은 날짜 형식이면 → `400 { error: 'Invalid input' }`
- 성공 시 → `201 { up: N, down: N }` (업데이트된 집계 반환, 프론트엔드 별도 GET 불필요)
...
- `yesterday.prediction`: 다수 득표 방향. 동률인 경우 `'up'`으로 기본 처리
- `yesterday.actual`: doom_records에서 어제(target_date - 1) vs 그저께(target_date - 2) total_score 비교. 두 기록 중 하나라도 없으면 `null`
...
// 오늘 집계
getVotesToday(targetDate) → { up: N, down: N }

// 어제 집계 + 실제 결과
getYesterdayVoteResult(yesterdayDate, dayBeforeDate) → { up, down, prediction, actual, correct } | null
```
